"""
verify_phase8_chain.py — Playwright smoke test for the FOPAG and Financeiro integration chain.

Usage:
    python scripts/verify_phase8_chain.py [--port PORT] [--base-url URL]

Environment variables:
    JOGAB_PORT      Port where the dev server is listening (default: 5173)
    JOGAB_BASE_URL  Full base URL override (takes precedence over JOGAB_PORT)
"""

import argparse
import asyncio
import os
from playwright.async_api import async_playwright


def get_base_url(args: argparse.Namespace) -> str:
    if args.base_url:
        return args.base_url.rstrip('/')
    port = args.port or int(os.getenv('JOGAB_PORT', '5173'))
    return f'http://localhost:{port}'


async def run(base_url: str) -> None:
    async with async_playwright() as p:
        browser = await p.chromium.launch()
        page = await browser.new_page(viewport={'width': 1280, 'height': 800})

        print(f"Using base URL: {base_url}")

        # Login
        await page.goto(f"{base_url}/login")
        await page.fill('input[type="email"]', 'admin@jogab.com.br')
        await page.fill('input[type="password"]', 'jogab123')
        await page.click('button[type="submit"]')

        # Wait for navigation to dashboard
        await page.wait_for_url(f"{base_url}/dashboard")

        # 1. FOPAG List
        print("Navigating to FOPAG List")
        await page.goto(f"{base_url}/fopag", wait_until="networkidle")
        await asyncio.sleep(2)
        await page.screenshot(path='fopag_list.png')
        print("Screenshot saved to fopag_list.png")

        # 2. FOPAG Detail
        print("Navigating to FOPAG Detail")
        try:
            await page.click('text=Abrir detalhe da competência')
            await page.wait_for_load_state("networkidle")
            await asyncio.sleep(2)
            await page.screenshot(path='fopag_detail.png')
            print("Screenshot saved to fopag_detail.png")
        except Exception as e:
            print(f"Warning: could not open FOPAG detail: {e}")

        # 3. Financeiro Dashboard
        print("Navigating to Financeiro Dashboard")
        await page.goto(f"{base_url}/financeiro", wait_until="networkidle")
        await asyncio.sleep(2)
        try:
            await page.click('text=Custo de pessoal por obra')
            await asyncio.sleep(1)
        except Exception:
            pass
        await page.screenshot(path='financeiro_dashboard.png')
        print("Screenshot saved to financeiro_dashboard.png")

        await browser.close()
        print("Verification completed.")


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Verify FOPAG and Financeiro integration chain.')
    parser.add_argument('--port', type=int, default=None, help='Dev server port (default: JOGAB_PORT env or 5173)')
    parser.add_argument('--base-url', default=None, help='Full base URL (overrides --port)')
    args = parser.parse_args()

    asyncio.run(run(get_base_url(args)))
