...
export function FuncionarioHorasExtrasPage() {
  ...
  const filtered = useMemo(() => {
    ...
    return items.filter((item) => {
      const matchesSearch =
        !search.trim() || `${item.obraNome ?? ''} ${item.tipo}`.toLowerCase().includes(search.trim().toLowerCase());
  ...
  return (
    <div className="flex flex-1 flex-col">
      <MainContent className="space-y-6">
        ...
        {isLoading && (
          <div className="py-12 text-center text-sm text-text-muted">Carregando horas extras do funcionário...</div>
        )}
        {isError && (
          <EmptyState
            title="Erro ao carregar horas extras"
            description="Não foi possível carregar os eventos de horas extras deste funcionário."
            action={
              <button
                type="button"
          ...
          <>
            ...
            {filtered.length === 0 ? (
              <EmptyState
                ...
                description="Não há horas extras deste funcionário para o filtro atual."
              />
            ) : (
              <FuncionarioWorkspaceTable
