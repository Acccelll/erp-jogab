...
export function FuncionarioFopagPage() {
  ...
  const filtered = useMemo(() => {
    ...
    return items.filter((item) => {
      const matchesSearch = !search.trim() || `${item.evento} ${item.origem} ${item.tipo}`.toLowerCase().includes(search.trim().toLowerCase());
  ...
  return (
    <div className="flex flex-1 flex-col">
      <MainContent className="space-y-6">
        ...
        {isLoading && <div className="py-12 text-center text-sm text-gray-500">Carregando eventos de FOPAG do funcionário...</div>}
        {isError && <EmptyState title="Erro ao carregar FOPAG" description="Não foi possível carregar os eventos de folha deste funcionário." action={<button type="button" onClick={() => void refetch()} className="rounded-md bg-jogab-500 px-3 py-1.5 text-sm text-white">Tentar novamente</button>} />}
        ...
        {!isLoading && !isError && data && (
          <>
            ...
            {filtered.length === 0 ? (
              <EmptyState title="Nenhum evento encontrado" description="Não há eventos de FOPAG deste funcionário para o filtro atual." />
            ) : (
              <FuncionarioWorkspaceTable
                columns={['Competência', 'Evento', 'Tipo', 'Origem', 'Status', 'Valor']}
                rows={filtered.map((item) => [formatCompetencia(item.competencia), item.evento, TIPO_LABELS[item.tipo], ORIGEM_LABELS[item.origem], STATUS_OPTIONS.find((option) => option.value === item.status)?.label ?? item.status, formatCurrency(item.valor)])}
              />
            )}
          </>
        )}
