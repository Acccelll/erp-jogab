...
export function FuncionarioDocumentosPage() {
  ...
  const filtered = useMemo(() => {
    ...
    return items.filter((item) => {
      const matchesSearch =
        !search.trim() ||
        `${item.codigo} ${item.titulo} ${item.responsavel}`.toLowerCase().includes(search.trim().toLowerCase());
  ...
  return (
    <div className="flex flex-1 flex-col">
      <MainContent className="space-y-6">
        ...
        {isLoading && (
          <div className="py-12 text-center text-sm text-text-muted">Carregando documentos do funcionário...</div>
        )}
          ...
          <>
            ...
            {filtered.length === 0 ? (
              <EmptyState
                ...
                description="Não há documentos deste funcionário para o filtro atual."
              />
            ) : (
              <FuncionarioWorkspaceTable
