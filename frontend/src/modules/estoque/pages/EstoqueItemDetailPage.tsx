          ...
          <>
              ...
              <article className="rounded-xl border border-gray-200 bg-white p-5 shadow-sm shadow-gray-100/60">
                <div className="flex flex-wrap items-start justify-between gap-4">
                  <div>
                    ...
                    <p className="mt-2 text-sm text-gray-500">{ESTOQUE_TIPO_LABELS[data.item.tipo]} • {data.item.unidade} • Competência {formatCompetencia(data.item.competencia)}</p>
                  </div>
                ...
                <div className="mt-5 grid gap-3 md:grid-cols-2 xl:grid-cols-4">
                  <div className="rounded-lg border border-gray-100 bg-gray-50 px-4 py-3">
                    ...
                    <p className="mt-1 text-lg font-semibold text-gray-900">{data.item.saldo.quantidadeAtual} {data.item.unidade}</p>
                  </div>
                  <div className="rounded-lg border border-gray-100 bg-gray-50 px-4 py-3">
                    <p className="text-xs uppercase tracking-wide text-gray-400">Disponível</p>
                    <p className="mt-1 text-lg font-semibold text-gray-900">{data.item.saldo.quantidadeDisponivel} {data.item.unidade}</p>
                  </div>
                  <div className="rounded-lg border border-gray-100 bg-gray-50 px-4 py-3">
                    <p className="text-xs uppercase tracking-wide text-gray-400">Reservado</p>
                    <p className="mt-1 text-lg font-semibold text-gray-900">{data.item.saldo.quantidadeReservada} {data.item.unidade}</p>
                  </div>
                  <div className="rounded-lg border border-gray-100 bg-gray-50 px-4 py-3">
                    <p className="text-xs uppercase tracking-wide text-gray-400">Valor total</p>
                    <p className="mt-1 text-lg font-semibold text-gray-900">{formatCurrency(data.item.saldo.valorTotal)}</p>
                  </div>
                </div>

                <div className="mt-5 grid gap-4 md:grid-cols-2">
                  <div className="rounded-lg border border-gray-100 bg-gray-50 px-4 py-3 text-sm text-gray-600">
                    <p><strong className="text-gray-900">Obra:</strong> {data.item.obraNome}</p>
                    <p><strong className="text-gray-900">Local:</strong> {data.item.localNome}</p>
                    <p><strong className="text-gray-900">Centro de custo:</strong> {data.item.centroCusto}</p>
                    <p><strong className="text-gray-900">Cobertura:</strong> {data.item.saldo.coberturaDias} dias</p>
                  </div>
                  <div className="rounded-lg border border-gray-100 bg-gray-50 px-4 py-3 text-sm text-gray-600">
                    <p className="flex items-center gap-2"><ShoppingCart size={14} className="text-jogab-600" /> {data.item.pedidoCompraCodigo ?? 'Sem pedido vinculado'}</p>
                    <p className="mt-2 flex items-center gap-2"><Receipt size={14} className="text-jogab-600" /> {data.item.documentoFiscal ?? 'Sem documento fiscal vinculado'}</p>
                    <p className="mt-2 flex items-center gap-2"><Wallet size={14} className="text-jogab-600" /> {data.item.tituloFinanceiroCodigo ?? 'Sem título financeiro vinculado'}</p>
                  </div>
                </div>
              </article>
            ...
            <section className="space-y-4">
              <div>
                ...
                <p className="text-sm text-gray-500">Leitura de apropriação operacional preparada para apoiar custos realizados da obra e futuras medições.</p>
              </div>
            ...
            <section className="space-y-4">
              <div>
                ...
                <p className="text-sm text-gray-500">Histórico de entradas, saídas, ajustes e transferências com vínculo de origem e responsável.</p>
              </div>
