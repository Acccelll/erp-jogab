import type { HoraExtraResumoCard } from '../types';

interface HorasExtrasResumoCardProps {
  card: HoraExtraResumoCard;
}

export function HorasExtrasResumoCard({ card }: HorasExtrasResumoCardProps) {
  return (
      ...
      <dl className="mt-4 space-y-3">
        {card.itens.map((item) => (
          <div key={item.label} className="flex items-center justify-between gap-4 rounded-lg bg-gray-50 px-3 py-2.5">
            ...
            <dd className={item.destaque ? 'text-sm font-semibold text-jogab-700' : 'text-sm font-semibold text-gray-800'}>
              {item.valor}
            </dd>
          </div>
        ))}
