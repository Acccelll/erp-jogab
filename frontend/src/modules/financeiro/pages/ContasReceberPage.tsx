...
export function ContasReceberPage() {
  ...
  return (
    <div className="flex flex-1 flex-col">
      ...
      />
      <MainContent>
        {isLoading && <p className="text-sm text-gray-500">Carregando contas a receber...</p>}
        {isError && (
          <EmptyState
            title="Erro ao carregar contas a receber"
            description="Não foi possível listar as entradas financeiras previstas/realizadas."
        ...
        {!isLoading && !isError && (
          <div className="space-y-4">
            <div className="rounded-xl border border-gray-200 bg-white p-4 shadow-sm shadow-gray-100/60">
              <h2 className="text-base font-semibold text-gray-900">Pipeline de recebimento</h2>
              <p className="mt-1 text-sm text-gray-500">Títulos a receber consolidam medições, faturamento e contratos para apoiar a previsão de caixa da obra.</p>
            </div>
            {!data || data.length === 0 ? (
              <EmptyState
