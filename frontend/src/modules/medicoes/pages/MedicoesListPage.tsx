        ...
        {!isLoading && !isError && data && (
          <>
            <section className="grid gap-4 xl:grid-cols-3">
              {data.resumoCards.map((card) => <MedicoesResumoCard key={card.id} card={card} />)}
            </section>
            ...
            {data.medicoes.length === 0 ? (
              <EmptyState
                ...
                description={hasActiveFilters ? 'Nenhuma medição corresponde aos filtros selecionados.' : 'Ainda não há medições disponíveis para o contexto atual.'}
              ...
              <section className="space-y-4">
                <div className="flex items-center justify-between gap-3">
                  <div>
                    ...
                    <p className="text-sm text-gray-500">Leitura consolidada do avanço medido por obra, contrato, aprovação e faturamento.</p>
                  </div>
                </div>
