...
export function ObraDocumentosPage() {
  ...
  const filtered = useMemo(() => {
    ...
    return items.filter((item) => {
      const matchesSearch =
        !search.trim() ||
        `${item.codigo} ${item.titulo} ${item.tipo} ${item.responsavel}`
          .toLowerCase()
  ...
  return (
    <div className="flex flex-1 flex-col">
      <MainContent className="space-y-6">
        ...
        {isLoading && <div className="py-12 text-center text-sm text-text-muted">Carregando documentos da obra...</div>}
        {isError && (
          <EmptyState
            title="Erro ao carregar documentos"
            description="Não foi possível carregar os documentos desta obra."
            action={
              <button
                type="button"
          ...
          <>
            ...
            {filtered.length === 0 ? (
              <EmptyState
                ...
                description="Não há documentos desta obra para o filtro atual."
              />
            ) : (
              <ObraWorkspaceTable
