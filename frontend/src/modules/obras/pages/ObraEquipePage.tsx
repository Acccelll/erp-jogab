...
export function ObraEquipePage() {
  ...
  const filtered = useMemo(() => {
    ...
    return items.filter((item) => {
      const matchesSearch =
        !search.trim() ||
        `${item.nome} ${item.funcao} ${item.equipe} ${item.matricula} ${item.centroCustoNome ?? ''}`
          .toLowerCase()
    ...
    <div className="flex flex-1 flex-col">
      <MainContent className="space-y-6">
        <ObraWorkspaceSectionHeader
          ...
          description="Pessoas e alocações consumidas da base de RH, preservando vínculo com funcionário e centro de custo."
        ...
        {!isLoading && !isError && data && (
          <>
            ...
            {filtered.length === 0 ? (
              ...
              <ObraWorkspaceTable
                columns={['Funcionário', 'Função', 'Equipe', 'Centro de custo', 'Status', 'Jornada']}
                rows={filtered.map((item) => [
                  `${item.nome} · ${item.matricula}`,
                  `${item.cargo} / ${item.funcao}`,
                  item.equipe,
                  item.centroCustoNome ?? 'Não vinculado',
